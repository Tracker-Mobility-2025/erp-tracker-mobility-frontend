/**
 * Manejador centralizado de errores para el módulo de verificadores.
 ...
 */
export class VerifierErrorHandler {
  /**
  ...
  /**
   * Maneja errores de operaciones con verificadores.
  ...
  handleHttpError(error, context) {
    ...
    // Error 409 - Conflict
    if (status === 409) {
      const message = data.message || `Ya existe un verificador con esos datos`;
  ...
  logError(error, context) {
    console.error(`[VerifierErrorHandler] Error al ${context}:`, {
      message: error.message,
      stack: error.stack,
