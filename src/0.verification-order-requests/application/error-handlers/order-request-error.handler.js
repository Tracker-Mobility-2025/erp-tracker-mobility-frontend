...
export class OrderRequestErrorHandler {
  ...
  /**
   * Maneja errores de operaciones con solicitudes de orden.
   * @param {Error} error - Error a manejar
   * @param {string} context - Contexto de la operación ('crear solicitud', 'actualizar', 'cargar')
  ...
  handleHttpError(error, context) {
    ...
    if (status === 403) {
      this.notificationService.showError(
        'No tiene permisos para crear solicitudes de verificación',
        'Acceso denegado',
        4000
      );
      return {
    ...
    // Error 409 - Conflict
    if (status === 409) {
      const message = data.message || 'Ya existe una solicitud con esos datos';
  ...
  logError(error, context) {
    console.error(`[OrderRequestErrorHandler] Error al ${context}:`, {
      message: error.message,
      stack: error.stack,
